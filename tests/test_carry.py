...
from carry import Carry, CarryStats
...
def test_pickup_then_walk_with_contact_only_payload(assets):
    ...
    assert result['payload_travel_m'] > .5
...
def mesh_bounds(sim, name):
    ...
    points = m.mesh_vert[start:start+count] @ d.geom_xmat[geom].reshape(3,3).T + d.geom_xpos[geom]
...
def test_original_meshes_match_payload_and_table_collisions():
    ...
    assert sim.platform_top == pytest.approx(.9)


def test_carry_input_validation_and_initial_report():
    for speed in [0, -.1, .31, float('nan')]:
