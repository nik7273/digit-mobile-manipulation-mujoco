...
from demo import DEFAULT_SCENE, ROOT, Simulation, load_controller
...
def test_scene_and_reset_preserve_objects():
    ...
    np.testing.assert_array_equal(sim.data.qpos[package:package+7], sim.model.qpos0[package:package+7])
    sim.step()
    sim.reset()
...
def test_mapping_includes_arms_and_actual_torques():
    ...
    d.ctrl[sim.actuator_ids] = np.linspace(-.1, .1, 20)
...
def test_velocity_convention_matches_alip():
    ...
    quat = np.empty(4)
    mujoco.mju_euler2Quat(quat, np.array([.2, -.3, .7]), 'xyz')
...
def test_cli_outside_repository(tmp_path):
    result = subprocess.run([sys.executable, str(ROOT / 'demo.py'), '--headless', '--duration', '.005'],
